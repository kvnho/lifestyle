...
export default class CarProgress extends React.Component {
	componentWillMount() {
		...
		this.setState({
			monthly: (parseFloat(this.props.weeklyRounded.substring(0, this.props.weeklyRounded.indexOf('.'))) * 30) / 7
		});
	}
				...
				<Grid container spacing={40}>
					...
					<Grid xs={1}>
						<h1
							...
							onMouseEnter={() => this.doHover(true, true)}
							onMouseLeave={() => this.doHover(true, false)}
							style={{ color: '#ddd', fontSize: 100, margin: 0, textAlign: 'right', marginTop: carH }}
						>
							{lc}
						</h1>
					</Grid>
					<Grid xs={3}>
						<div style={{ width: fullCarW, height: carH, paddingLeft: 50 }}>
							<div
								style={{
									zIndex: 100,
									width: fullCarW * this.getFuturePercent(),
									height: carH,
									overflow: 'hidden',
									position: 'absolute'
								}}
							>
								<img
									src={this.chooseFutureImage()}
									alt='Car'
									style={{ width: fullCarW, height: carH }}
								/>
							</div>
							<div styes={{ position: 'absolute', zIndex: 200 }}>
								<img
									src={this.chooseFutureImage()}
									alt='Car'
									style={{ width: fullCarW, height: carH, opacity: 0.6 }}
								/>
							</div>
						</div>

						<h1 style={{ color: '#999', fontSize: 20, margin: 0 }}>After {this.state.months} months:</h1>
						<h1 style={{ color: 'rgb(180, 229, 100)', fontSize: 70, margin: 0 }}>
							${Math.ceil(this.state.months * this.state.monthly + saved)}
						</h1>
						<h1 style={{ color: '#999', fontSize: 16, margin: 0 }}>
							If you continue rounding up ${Math.ceil(this.props.weeklyRounded)} per week
						</h1>
					</Grid>
					<Grid xs={1}>
						<h1
							id='rc'
							onClick={() => this.moveMonth(true)}
							onMouseEnter={() => this.doHover(false, true)}
							onMouseLeave={() => this.doHover(false, false)}
							style={{ color: '#ddd', fontSize: 100, margin: 0, marginTop: carH, textAlign: 'left' }}
						>
							{rc}
						</h1>
					</Grid>
